...
class ImagePickerActivity : AppCompatActivity() {
    ...
    private var mFileProvider: FileProvider? = null
    ...
    private fun loadBundle(savedInstanceState: Bundle?) {
        ...
        when (intent?.getSerializableExtra(ImagePicker.EXTRA_IMAGE_PROVIDER) as ImageProvider?) {
            ...
            ImageProvider.FILE -> {
                mFileProvider = FileProvider(this) { fileLauncher.launch(it) }
                // Pick Gallery Image
                savedInstanceState ?: mFileProvider?.startIntent()
            }
            else -> {
